...
struct Lidar
{
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope)   // 라이다 설정
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		// TODO:: set minDistance to 5 to remove points from roof of ego car
		minDistance = 5;         // noise setting
		maxDistance = 50;
		resoultion = 0.2;
		// TODO:: set sderr to 0.2 to get more interesting pcd files (0.2m)
		sderr = 0.2;
		cars = setCars;
		...
		double horizontalAngleInc = pi/64;        // ch 변경 
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr scan()     // scanfunction : create a point cloud
	{
		cloud->points.clear();
